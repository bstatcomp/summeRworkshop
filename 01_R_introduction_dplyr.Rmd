...
it only prints as many columns as it can fit into a page, and lists other
columns below. If we wish to see the whole tibble, we can use the function
`View()`. Under the variable names, a tibble shows the type of the variables. 
...
## Filter
The function `filter()` allows us to select rows, based on variable values.
Input to the function is a tibble plus the conditions and the output is a new
...
## Mutate
To create new variables in the data frame, dependent on existing variables,
...
We can also use custom functions when creating new variables. 
...
without creating an additional data set, or changing the original.
...
Usually we encounter data in the wide format. The wide format of data is
...
(for example `ggplot`) require the long data format.
